...
UCLASS()
class KI7_UNREALNETWORK_JM_API AReplicationCharacter : public ACharacter
{
	GENERATED_BODY()
...
protected:
	...
	UPROPERTY(ReplicatedUsing = OnRepNotify_Level)	// Level이 리플리케이션 될 때마다 OnRepNotify_Level이 실행
	int32 Level = 1;
	...
	float Exp = 0.0f;		// 리플리케이션 안되는 일반 변수

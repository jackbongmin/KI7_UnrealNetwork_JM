...
void AReplicationCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION(AReplicationCharacter, Level, COND_OwnerOnly);	// Owner에게만 리플리케이션
	DOREPLIFETIME_CONDITION(AReplicationCharacter, Exp, COND_SimulatedOnly);// Owner를 제외한 다른사람들에게만 리플리케이션
	DOREPLIFETIME(AReplicationCharacter, Health);							// Health는 모두에게 리플리케이션
